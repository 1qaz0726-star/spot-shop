          ...
          <div className="flex flex-wrap gap-2 px-1 pb-1">
            {hotSearches.map((term) => (
              <button
                ...
                onMouseDown={(e) => e.preventDefault()}
                onClick={() => onSelect(term)}
                className="nc-interactive nc-press rounded-full border border-gray-200 bg-gray-50 px-3 py-1.5 text-sm text-gray-700 hover:border-[var(--color-brand)]/30 hover:bg-[var(--color-brand-light)] hover:text-[var(--color-brand)]"
              >
                {term}
              </button>
            ))}
